...
PUT /api/secondary-workflows/{id}/config persists a workflow's global
...
async def test_put_persists_and_echoes_effective_config(client):
    ...
    resp = await client.put(
        "/api/secondary-workflows/cfg_a/config",
        json={"config": {"style": "bright", "depth": 3}},
    )
    assert resp.status_code == 200
    assert resp.json() == {"config": {"style": "bright", "depth": 3}}
    got = await client.get("/api/secondary-workflows/cfg_a/config")
...
async def test_get_unset_slot_returns_defaults(client):
    register_workflow(Workflow(id="cfg_a", display_name="A", config_defaults={"style": "noir"}))
    resp = await client.get("/api/secondary-workflows/cfg_a/config")
    assert resp.status_code == 200
...
async def test_empty_config_clears_slot_and_restores_defaults(client):
    register_workflow(Workflow(id="cfg_a", display_name="A", config_defaults={"style": "noir"}))
    await client.put("/api/secondary-workflows/cfg_a/config", json={"config": {"style": "bright"}})
    resp = await client.put("/api/secondary-workflows/cfg_a/config", json={"config": {}})
    assert resp.status_code == 200
    assert resp.json() == {"config": {"style": "noir"}}
    got = await client.get("/api/secondary-workflows/cfg_a/config")
...
async def test_unregistered_workflow_404(client):
    put = await client.put("/api/secondary-workflows/ghost/config", json={"config": {"x": 1}})
    assert put.status_code == 404
    get = await client.get("/api/secondary-workflows/ghost/config")
...
async def test_bad_body_422_leaves_slot_unchanged(client):
    register_workflow(Workflow(id="cfg_a", display_name="A", config_defaults={"style": "noir"}))
    await client.put("/api/secondary-workflows/cfg_a/config", json={"config": {"style": "bright"}})
    missing = await client.put("/api/secondary-workflows/cfg_a/config", json={})
    assert missing.status_code == 422
    non_dict = await client.put("/api/secondary-workflows/cfg_a/config", json={"config": [1, 2]})
...
async def test_per_slot_isolation(client):
    ...
    await client.put("/api/secondary-workflows/cfg_a/config", json={"config": {"only": "a"}})
    await client.put("/api/secondary-workflows/cfg_b/config", json={"config": {"only": "b"}})
    a = await client.get("/api/secondary-workflows/cfg_a/config")
    b = await client.get("/api/secondary-workflows/cfg_b/config")
...
async def test_put_write_serializes_under_config_lock(client):
    ...
    async with workflow_config_lock():
        task = asyncio.create_task(client.put("/api/secondary-workflows/cfg_a/config", json={"config": {"start": 2}}))
